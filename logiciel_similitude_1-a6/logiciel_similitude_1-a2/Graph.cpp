...
void CGraph::SetXYScale(int sX, int sY)
{
	this->Xscale = sX;
	this->Yscale = sY;
}
...
void CGraph::DrawGraph(CDC* pDC,int x,int y, int w,int h) // fonction modifier pour le logiciel
{
	CString tickLabel;
	CWnd* graphWnd = pDC->GetWindow();
	CRect graphRect;
	//graphWnd->GetClientRect(&graphRect);
	graphRect.SetRect(0,0,w,h);
	graphRect.MoveToXY(x,y);
	TEXTMETRIC	tm;
	...
	CBrush* pOldBackBrush;
	pOldBackBrush = pDC->SelectObject(&backBrush);
	pDC->Rectangle(x, y, x+ graphRect.Width(), y+graphRect.Height());
	pDC->SelectObject(pOldBackBrush);
	...
	int charWidth = tm.tmAveCharWidth;
	//next line is centered....trust me !!
	pDC->TextOut(((x+graphRect.Width()) / 2) - ((graphTitle.GetLength() / 2) * charWidth),
				y+10, graphTitle);
	pDC->SelectObject(pOldFont);
	...
	if((graphType == PIE_GRAPH) || (graphType == PIE_GRAPH_3D))
	{
		...
		//since pie does not have axis lines, set to full size minus 5 on each side
		//these are needed for legend to plot itself
		xAxisWidth = x+maxWidth - 10;
		yAxisHeight = y+maxHeight - 50;  //10 buffer, 20 for title, and 20 for series label
		xApexPoint = x + 5;
		yApexPoint = y + maxHeight - 5;
	}
	else
	{
		//compute label size for axis alignment
		...
		if(graphAlignment)
		{
			if(graphType == SCATTER_GRAPH)
			{
				xApexPoint = x + (xAxisLabelLength * charWidth) + labelHeight; //labelHeight added for y-axis label height
				yApexPoint = y + (maxHeight - 15) - tm.tmHeight;
				yAxisHeight = yApexPoint - 40;
				xAxisWidth = x + (maxWidth - 5) - xApexPoint;
			}
			else
			{
				xApexPoint =x+  5 + (tickLabel.GetLength() * (yTickFontSize / 2)) + 45; 
				if(!xAxisAlign)  //horizontal
					yApexPoint = 2*y + (maxHeight - 5) - 45;		//apex points are the cross section of axis lines
				else
					yApexPoint = 2*y + (maxHeight - 5) - (xAxisLabelLength * (xTickFontSize / 2)) - 10;
				yAxisHeight = yApexPoint -40-y;
				xAxisWidth = x + (maxWidth - 5) - xApexPoint;
			}
		}
		else
		{
			xApexPoint = x + 5 + (xAxisLabelLength * charWidth) + labelHeight; //labelHeight added for y-axis label height
			yApexPoint = y+(maxHeight - 15) - tm.tmHeight;
			yAxisHeight = y+yApexPoint - 40;
			xAxisWidth = (maxWidth - 5) - xApexPoint;
		}
	}
...
void CGraph::DrawAxis(CDC* pDC,int x_e,int y_e)
{
	TEXTMETRIC	tm;
		...
		case XY_LINE_GRAPH :
				...
				if(graphAlignment)
				{
					switch(graphQuadType)
					{
						case 1 :
						case 2 :
							pDC->TextOut(10 + x_e, (yApexPoint - (yAxisHeight / 2)) + ((axisYLabel.GetLength() * charWidth) / 2), axisYLabel);
							break;
						case 3 :
						case 4 :
							pDC->TextOut(10 + x_e , yApexPoint + ((axisYLabel.GetLength() * charWidth) / 2), axisYLabel);
							break;
					}
					...
					{
						case 1 :
							pDC->TextOut(xApexPoint + (xAxisWidth / 2) - ((axisXLabel.GetLength() / 2) * charWidth), maxHeight - 5 - 6 + y_e, axisXLabel);
							break;
						case 2 :
					...
					tickScale = 0.00;
					tickScale = (yAxisHeight - 10) / numYTicks;
						...
						switch(graphQuadType)
						{
							case 1 :
								for(y = 1; y <= numYTicks; y++) 
								{
									...
									//draw tick label
									CString tickLabel;
									if((this->Yscale != 100)&&(this->Yscale != 1000)&&(this->Yscale != 10000)&&(this->Yscale != 100000))
										tickLabel.Format(L"%d", minTick + (y * tickYSpace));
									else
										tickLabel.Format(L"%3f",float (float (minTick) + (float (y) * float( float(tickYSpace) / float(this->Yscale)))));
							...
							case 2 :
								if(graphType == SCATTER_GRAPH)
								{
									...
									for(y = 1; y <= numTicks; y++)  
									{
										...
										//draw tick label
										CString tickLabel;
										tickLabel.Format(L"%d", minTick + (y * tickSpace));
									...
									//draw the 0
									pDC->TextOut(xApexPoint - (xAxisWidth / 2) - 10 - (yTickFontSize / 2), yApexPoint - 6, L"0");
							...
							case 3 :
								for(y = 0; y <= numTicks / 2; y++) 
								{
									...
									//draw tick label
									CString tickLabel;
									tickLabel.Format(L"%d", maxTick - (y * tickSpace));
								...
								for(; y <= numTicks; y++) 
								{
									...
									//draw tick label
									CString tickLabel;
									tickLabel.Format(L"%d", minTick + ((numTicks - y) * tickSpace));
						...
						case 1 :
							...
							else
							{
								...
								for(int x = 1; x <= numXTicks; x++)  
								{
									...
									if((this->Xscale != 100)&&(this->Xscale != 1000)&&(this->Xscale != 10000)&&(this->Xscale != 100000))
										tickLabel.Format(L"%d", x * tickXSpace);
									else
										tickLabel.Format(L"%f", float (x) * float (float(tickXSpace)/ float(this->Xscale)));
							...
							else
							{
								...
								for(x = 0; x <= numTicks / 2; x++)  
								{
									...
									if(!inRedraw)
									{
										//draw tick label
										CString tickLabel;
										tickLabel.Format(L"%d", minTick + (x * tickSpace));
								...
								if(!inRedraw)
								{
									...
									pDC->TextOut(tickXLocation - (charWidth / 2), yApexPoint + 8, L"0");
								}
								...
								for(; x <= numTicks; x++)  
								{
									...
									if(!inRedraw)
									{
										//draw tick label
										CString tickLabel;
										tickLabel.Format(L"%d", (x / 2) * tickSpace);
							...
							else
							{
								...
								for(int x = 1; x <= numTicks; x++)  
								{
									...
									if(!inRedraw)
									{
										//draw tick label
										CString tickLabel;
										tickLabel.Format(L"%d", x * tickSpace);
				...
				else
				{
					...
					switch(graphQuadType)
					{
						case 1 :
							...
							if(axisYLabel.GetLength() == 0)
								labelHeight = 0;
							pDC->TextOut(xApexPoint + x_e + (xAxisWidth / 2) - ((axisXLabel.GetLength() / 2) * charWidth), maxHeight - 5 - 6, axisYLabel);
							...
							if(graphType != SCATTER_GRAPH)
							{
								...
								for(y = 1; y <= graphSeries->GetCount(); y++)  
								{
									...
									if(!inRedraw)
									{
										...
										yFont.CreateFont(yTickFontSize, 0, 0, 0, 700, FALSE, FALSE, 0,
											ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
											DEFAULT_QUALITY, FIXED_PITCH | FF_ROMAN,L"Arial");
										pDC->SelectObject(&yFont);
										charWidth = tm.tmAveCharWidth;
						...
						case 2 :
							...
							if(!inRedraw)
							{
								...
								else
								{
									...
									charWidth = tm.tmAveCharWidth;
									if(xAxisAlign < 180)
										pDC->TextOut(tickXLocation - 8, yApexPoint + 8 + (xAxisLabelLength * charWidth), L"0");
									else
										pDC->TextOut(tickXLocation + 2, yApexPoint + 8, L"0");
						...
						case 3 :
							...
							else
							{
								...
								if(!inRedraw)
								{
									...
									pDC->TextOut(xApexPoint - (yTickFontSize / 2) - 8, tickYLocation - 6, L"0");
									pDC->SelectObject(pOldFont);
								}
								y++;
							...
							for(x = 0; x <= numTicks; x++)
							{
								...
								if(!inRedraw)
								{
									...
									//draw tick label
									CString tickLabel;
									tickLabel.Format(L"%d", (x * tickSpace * 2));
									if(!xAxisAlign)  //horizontal
									{
										CFont xFont;
										xFont.CreateFont(xTickFontSize, 0, 0, 0, 700, FALSE, FALSE, 0,
						...
						case 4 :
							...
							if(!inRedraw)
							{
								...
								if(!xAxisAlign)  //horizontal
								{
									...
									pDC->SelectObject(&xFont);
									charWidth = tm.tmAveCharWidth;
									pDC->TextOut(tickXLocation - (charWidth / 2), yApexPoint + (yAxisHeight / 2) + 8, L"0");
									pDC->SelectObject(pOldFont);
								}
								else
								{
									...
									charWidth = tm.tmAveCharWidth;
									if(xAxisAlign < 180)
										pDC->TextOut(tickXLocation - 8, yApexPoint + (yAxisHeight / 2) + 8 + (xAxisLabelLength * charWidth), L"0");
									else
										pDC->TextOut(tickXLocation + 2, yApexPoint + (yAxisHeight / 2) + 8, L"0");
...
//				 break;
		...
		case PIE_GRAPH_3D :
				 ...
				 break;
		default :
				 AfxMessageBox(L"No graph type to display");
				 break;
	}
...
//				 break;
		...
		case PIE_GRAPH_3D :
				 ...
				 break;
		default :
				 AfxMessageBox(L"No graph type to print");
				 break;
	}
}
...
void CGraph::SetTickLimitsXY(int minXTickValue, int maxXTickValue, int XtickStep,int minYTickValue, int maxYTickValue, int YtickStep)
{
	//X
	maxXTick = maxXTickValue;
	minXTick = minXTickValue;
	tickXSpace = XtickStep;
	numXTicks = (maxXTick - minXTick) / XtickStep;
	if((numXTicks * XtickStep) < (maxXTick - minXTick))
		numXTicks++;
	//Y
	maxYTick = maxYTickValue;
	minYTick = minYTickValue;
	tickYSpace = YtickStep;
	numYTicks = (maxYTick - minYTick) / YtickStep;
	if((numYTicks * YtickStep) < (maxYTick - minYTick))
		numYTicks++;
}
/*void CGraph::SetTickLimitsXY(float minXTickValue,float maxXTickValue,float XtickStep,float minYTickValue,float maxYTickValue,float YtickStep)
{
/*	//X
	maxXTick = 10 * maxXTickValue;
	minXTick = 10 * minXTickValue;
	tickXSpace = 10 * XtickStep;
	numXTicks = (maxXTick - minXTick) / XtickStep;
	if((numXTicks * XtickStep) < (maxXTick - minXTick))
		numXTicks++;
	//Y
...
//there should never be a half point ending on 4, so leave it out
				...
				pieLabel->pieLabel.Format(L"%d%%", roundPercent);
...
void CGraph::DrawScatterSeries(CDC* pDC) // modifier pour le logiciel
{
	...
	{
		case 1 :
			yAxisScale = ((yApexPoint - topYTick) / (maxYTick * 1.00));
			xAxisScale = ((rightXTick - xApexPoint) / (maxXTick * 1.00));
			break;
		case 2 :
	...
	for(pos = graphSeries->GetHeadPosition(); pos != NULL; graphSeries->GetNext(pos))
	{
		...
		if((tmpSeries->GetXDataValue() != -1) && (tmpSeries->GetYDataValue() != -1)) // instruction donné dans la série pour effectué le changement de courbe
		{
			//multiply each value by 1.00 to force result into a double value, and therefore
			//make it more accurate in it's plot location.
			...
			if (x>1)
			{
				...
				pDC->LineTo(tickXLocation,tickYLocation);
				pDC->SelectObject(pOldPen);
		...
		else //remise a zero de la serie
		{
			x=1;
			i=1;
		}
